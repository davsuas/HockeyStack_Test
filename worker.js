...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    const meetingToAssociate = meetingIds;
    const meetingAssociationsResults = (await (await hubspotClient.apiRequest({
      ...
      body: { inputs: meetingToAssociate.map(meetingId => ({ id: meetingId })) }
    })).json())?.results || [];

    const contactAssociations = Object.fromEntries(meetingAssociationsResults.map(a => {
      if (a.from) {
        meetingToAssociate.splice(meetingToAssociate.indexOf(a.from.id), 1);
        return [a.from.id, a.to[0].id];
      } else return false;
    }).filter(x => x));
    ...
    data.forEach(meeting => {
      ...
      const meetingProperties = {
        ...
        meeting_video_call_url: meeting.properties.hs_video_conference_url
      };
